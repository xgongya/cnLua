...
static const char *const tokennames2[] = {
	"且", "跳出循环", "运行", "否则", "否则如果", "结束", "假", "循环", "函数", "跳转", "如果", "在", "局部", "空", "不是", "或", "循环判断尾", "返回", "则", "真", "判断尾", "判断循环", "..", "...", "==", ">=", "<=", "~=", "::", "<number>", "<name>", "<string>", "<eof>",
	NULL
};
...
void lj_lex_init(lua_State *L)
{
  ...
  for (i = 0; i < TK_RESERVED; i++) {
    GCstr *s = lj_str_newz(L, tokennames2[i]);
    fixstring(s);  /* Reserved words are never collected. */
    s->reserved = (uint8_t)(i+1);
